...
class ChatArea extends StatefulWidget {
  ...
  final String roomId;
  const ChatArea({super.key, required this.userName, required this.roomId});
...
class _ChatAreaState extends State<ChatArea>
  ...
  void _initializeSocketChat() {
    ...
    _socketService.getMessages(widget.roomId);

    _messageSubscription = _socketService.messages.listen((data) {
      if (data['room'] == widget.roomId) {
        if (data['type'] == 'message') {
          final newMessage = Chat.fromJSON(data['message']);
          if (newMessage.senderId != _socketService.socket?.id) {
            setState(() {
          ...
          setState(() {
            sendmessages = (data['messages'] as List)
  ...
  void sendChat() {
    if (controller.text.isNotEmpty) {
      ...
      final chat = Chat(
        ...
        senderId: _socketService.socket?.id ?? 'unknown',
      );
      ...
      _socketService.sendMessage(widget.roomId, chat.toJSON());
  ...
  void refreshChat() async {
    if (_socketService.socket?.connected == true) {
      Logger().i('Requesting messages for room: ${widget.roomId}');
      _socketService.getMessages(widget.roomId);
    } else {
  ...
  Future<void> voiceCall() async {
    if (isBlocked) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(
                'Unable to initiate call. User is blocked/has blocked you.')),
      );
    }
  }
          ...
          IconButton(
            ...
            onPressed: () {
              if (isBlocked) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                      content: Text('Unable to refresh. User is blocked.')),
                );
                return;
              }
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final bool isMe = message.senderId ==
                              (_socketService.socket?.id ?? 'unknown');
